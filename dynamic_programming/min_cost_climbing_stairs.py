...
Process:
For this problem we use dynamic programming (bottom-up approach)


We do a similar approach to fibonacci where we use the previous 2 iterations for our current since we can jump either 1 or 2. 

For this case since we want to find a min path from start to end, we can add our the current value from the list to the min of the previous 2. 
We do this because we jump either 1 or 2 times and we add the sum we accumalete when we jump.

Set the sum for the current iteration so you can use it for the next iteration when looking back. (you can see how this is using memoization by looking at previous computations)
